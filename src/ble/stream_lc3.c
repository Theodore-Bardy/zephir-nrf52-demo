...
static void fill_audio_buf_sin(struct tx_stream *stream) {
  const unsigned int num_samples =
      (stream->lc3_tx.frame_duration_us * stream->lc3_tx.freq_hz) /
      USEC_PER_SEC;
  const int sine_period_samples =
      stream->lc3_tx.freq_hz / AUDIO_TONE_FREQUENCY_HZ;
...
static bool encode_frame_block(struct tx_stream *stream,
                               struct net_buf *out_buf) {
  for (uint8_t i = 0U; i < stream->lc3_tx.chan_cnt; i++) {
    /* We provide the total number of decoded frames to `decode_frame` for
     * logging purposes
     */
...
int stream_lc3_init(struct tx_stream *stream) {
  ...
  /* Fill audio buffer with Sine wave only once and repeat encoding the same
   * tone frame */
  ...
  stream->lc3_tx.encoder =
      lc3_setup_encoder(stream->lc3_tx.frame_duration_us,
                        stream->lc3_tx.freq_hz, 0, &stream->lc3_tx.encoder_mem);
