...
static void tx_thread_func(void *arg1, void *arg2, void *arg3) {
  ...
  /* This loop will attempt to send on all streams in the streaming state in a
   * round robin fashion. The TX is controlled by the number of buffers
   * configured, and increasing CONFIG_BT_ISO_TX_BUF_COUNT will allow for more
   * streams in parallel, or to submit more buffers per stream. Once a buffer
   * has been freed by the stack, it triggers the next TX.
   */
  while (true) {
    int err = -ENOEXEC;
...
void stream_tx_init(void) {
  ...
  if (!thread_started) {
    ...
    k_thread_create(&tx_thread, tx_thread_stack,
                    ...
                    NULL, NULL, NULL, tx_thread_prio, 0, K_NO_WAIT);
