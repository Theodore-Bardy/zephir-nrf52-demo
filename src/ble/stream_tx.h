 ...
 * @brief Initialize TX
 *
 * This will initialize TX if not already initialized. This creates and starts a
 * thread that will attempt to send data on all streams registered with
 * stream_tx_register().
 */
 ...
 *
 * This will add it to the list of streams the TX thread will attempt to send
 * on.
 *
 * @retval 0 on success
 ...
 *
 * This will remove it to the list of streams the TX thread will attempt to send
 * on.
 *
 * @retval 0 on success
 * @retval -EINVAL if @p bap_stream is NULL
